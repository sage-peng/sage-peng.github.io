...
9. 调试工具（chatgpt给出）
   ...
   2. 层序遍历二叉树列表为[1,2,3,None,None,4,5]，还原成二叉树
      ...
      # 层序遍历二叉树列表为[1,2,3,None,None,4,5]，还原成二叉树        
...
### LeetCode 129.求根到叶子节点数字之和（⭐️⭐️高频、剑指II）-1
...
### ⭐️⭐️⭐️LeetCode98.验证二叉搜索树（⭐️⭐️高频、HOT100）-1
...
### ⭐️⭐️⭐️LRC155.[剑指offer36]: 二叉搜索树与双向链表（⭐️高频）-1
...
### LeetCode543.二叉树的直径 （⭐️⭐️高频、HOT100）-1
...
- 方法一:
  ...
  - 采用左中位数：左区间扩大（就留两个数想）,临界情况要分到左边（否则可能分不开）

  - 二分查找如果存在结果，则left和right都指向target

- 方法二：
  - 采用右中位数:右区间扩大（就留两个数想）,临界情况要分到右边（否则可能分不开）
- 方法三：旋转序列（仅LeetCode33）
- 总结：
  - 方法一、方法二最终left和right指向一处对于问题容易处理很多
  - 当有多个相同target时，方法一找到的是左边的，方法二找到的是右边的
  - 方法二找离目标最近的小值；方法一找离目标最近的大值；
...
### ⭐️⭐️LeetCode34.在排序数组中查找元素的第一个和最后一个位置（⭐️⭐️⭐️经典 高频、HOT100）-1
...
class Solution:
    def searchMatrix(self, matrix: List[List[int]], target: int) -> bool:
        ...
        while l<r:
            mid=l+(r-l)//2
            if target>matrix[mid//w][mid%w]:l=mid+1
            else:r=mid

        return True if matrix[l//w][l%w]==target else False
```

### LeetCode240.搜索二维矩阵II（⭐️⭐️高频、HOT100）-1

- 20240905

- 解法：
  - 从左上角出发都是增大的方向，所以改成从右上角出发
  - 时间复杂度是O(n),就是最多走2n-1步
...
### LeetCode33.搜索旋转排序数组（⭐️⭐️⭐️经典 难 高频、HOT100 腾讯）-1
...
### LeetCode153.搜索旋转数组的最小值（⭐️⭐️难 高频）-1

- 20240905
- 取左中值时，对比中值，相等为左区间；反之亦然；
...
### LeetCode154.搜索旋转数组的最小值II（⭐️⭐️难 高频）-1
...
[视频讲解](https://www.bilibili.com/video/BV1cv41117ve?spm_id_from=333.337.search-card.all.click)
    ...
    def findMin(self, nums: List[int]) -> int:
        ...
        while l<r:
            ...
            if nums[mid]>nums[r]:
                ...
                # excluding one is still exclusion
                r-=1
                continue
            else:
                r=mid
...
### LeetCode162.寻找峰值/极大值（⭐️高频）-1

- 20240906
- 二分法在这里有一个反例 [2,2,1,3]
  - 这一题要每一个元素都是distinct才完整
- 贪心法做不到O(log(n))

[LeetCode162.寻找峰值](https://leetcode.cn/problems/find-peak-element/)
[视频讲解](https://www.bilibili.com/video/BV1BU4y1T7ri?spm_id_from=333.337.search-card.all.click)
...
### ⭐️⭐️⭐️LeetCode4.寻找两个正序数组的中位数（⭐️⭐️⭐️经典 难 HOT100）-1
...
[题目视频讲解](https://www.bilibili.com/video/BV1z54y1b7wb?spm_id_from=333.337.search-card.all.click)
[题目精彩文字讲解](https://leetcode-cn.com/problems/median-of-two-sorted-arrays/solution/xiang-xi-tong-su-de-si-lu-fen-xi-duo-jie-fa-by-w-2/)

```python
class Solution:
    def findMedianSortedArrays(self, nums1: List[int], nums2: List[int]) -> float:
        def findKth(star1:int,end1:int,star2:int,end2:int,k:int) -> int:
            if star1>end1:return nums2[star2+k-1]
            if star2>end2:return nums1[star1+k-1]
            
            if k==1:
                return min(nums1[star1],nums2[star2])
            
            mid=k//2
            inx1=star1+mid-1 if end1>=star1+mid-1 else end1
            inx2=star2+mid-1 if end2>=star2+mid-1 else end2
            
            
            if nums1[inx1]<nums2[inx2]:
                return findKth(inx1+1,end1,star2,end2,k-(inx1+1-star1))   
            else:
                return findKth(star1,end1,inx2+1,end2,k-(inx2+1-star2))

        m,n=len(nums1),len(nums2)
        mid1,mid2=(m+n+1)//2,(m+n+2)//2
        return (findKth(0,m-1,0,n-1,mid1)+findKth(0,m-1,0,n-1,mid2))/2
```

### ⭐️⭐️LeetCode287. 寻找重复数（⭐️HOT100）-1

- 20240906
- 取左中值，那么临界值就分到左区间。因为在循环中单独不考虑临界值，只分成左右两个区间，临界值如果分到右边就有机会出现无法分开而导致死循环

[LeetCode287. 寻找重复数](https://leetcode-cn.com/problems/find-the-duplicate-number/)

[题解](https://leetcode-cn.com/problems/find-the-duplicate-number/solution/xun-zhao-zhong-fu-shu-by-leetcode-solution/)
...
### LeetCode20 有效的括号（⭐️⭐️⭐️基础题 经典 高频、HOT100 腾讯）-1
...
### LeetCode155. 最小栈（⭐️⭐️基础题 高频、HOT100 腾讯）-1

- 20240902
- 设计题：
  - 维护一个minStack，栈顶放当前stack的最小值
...
[视频讲解](https://www.bilibili.com/video/BV1W5411L7jC?spm_id_from=333.337.search-card.all.click)
...
class MyQueue:
    ...
    def empty(self) -> bool:
        return True if not self.stack_out and not self.stack_in else False
```

### LeetCode394. 字符串解码（⭐️经典 Hot100）-1

- 20240902
- 注意：存入[当前重复的次数，和之前的字符串]
    ...
    def decodeString(self, s: str) -> str:
        ...
        for c in s:
            ...
            if c=='[':
                ...
                k,res=0,"" 
            if c==']':
                cur_k,pre_res=stack.pop()
                res=pre_res+cur_k*res 
...
- ### 单调栈的特点
  ...
  1. **单调性**：
     - 单调栈中的元素具有单调性，即栈中的元素要么是单调递增的，要么是单调递减的。
  2. **元素的进出**：
     - 元素进入栈时，会根据单调性要求移除不符合单调性条件的元素。
     - 元素出栈时，通常移除栈顶元素。

### LeetCode739. 每日温度（⭐️Hot100）-1
...
### ⭐️⭐️⭐️LeetCode84. 柱状图中最大的矩形（⭐️⭐️⭐️经典 难 高频、HOT100）-1
...
- 理解:左边最小的位置
- 当前高度的最大矩形的宽，紧邻这左右第一个小于当前高度的坐标

[LeetCode84. 柱状图中最大的矩形](https://leetcode-cn.com/problems/largest-rectangle-in-histogram/)

[视频讲解](https://www.bilibili.com/video/BV1fi4y1d7YX?spm_id_from=333.999.0.0)
...
### ⭐️⭐️LeetCode85. 最大矩形（⭐️⭐️经典 难 HOT100）-1

- 20240903
- 这道题基本就是上题一样的解法
- 注意这一题用dp计算的时候当前（i,j）的面积不能光由（i-1,j）和（i,j-1）确定，所以不能计算
...
class Solution:
    def maximalRectangle(self, matrix: List[List[str]]) -> int:
        def largestRectagele(nums:List[int])->int:
            nums=[0]+nums+[0]
            stack=[]
            max_area=0
            for i in range(len(nums)):
        ...
        max_area=0       
        for i in range(h):
            max_area=max(max_area,largestRectagele(matrix[i]))
        
        return max_area
```

### ⭐️⭐️⭐️LeetCode42. 接雨水（⭐️⭐️⭐️经典 难 高频、HOT100）-1

- 20240903

[LeetCode42. 接雨水](https://leetcode-cn.com/problems/trapping-rain-water/)

[视频讲解 42. 接雨水 Trapping Rain Water 【LeetCode 力扣官方题解】](https://www.bilibili.com/video/BV1fi4y1t7BP?spm_id_from=333.337.search-card.all.click)
    ...
    def trap(self, height: List[int]) -> int:
        ...
        for i in range(l):
            while stack and height[i]>height[stack[-1]]:
                h=height[stack.pop()]
                if not stack :break
                # if h==height[stack[-1]]:continue
                h=min(height[stack[-1]],height[i])-h
                res+=h*(i-stack[-1]-1)
            stack.append(i)
        return res
```
...
### ⭐️⭐️⭐️Leetcode239. 滑动窗口最大值（⭐️⭐️⭐️经典 难 高频 HOT100）-1

- 20240903

[Leetcode239. 滑动窗口最大值](https://leetcode-cn.com/problems/sliding-window-maximum/)

[文字题解](https://zhongqiang.blog.csdn.net/article/details/115293314)
